...
/**
 * Settings → Radarr (COL-144), split off `settingsInstances.test.tsx`: the
 * type-scoped Radarr slice of `InstancesSection`'s CRUD behavior, minus the
 * type-selector assertions the old combined-table create form had (there's
 * no selector any more -- the page fixes `type="radarr"`).
 */
describe("InstancesSection (Radarr)", () => {
  afterEach(() => {
    vi.unstubAllGlobals();
  });

  it("lists instances from a mocked API response", async () => {
    mockInstancesApi([radarr]);
    render(<InstancesSection type="radarr" />);
  ...
  it("only lists Radarr instances, even when Sonarr instances are also configured", async () => {
  ...
  it("renders an empty state with no Radarr instances configured", async () => {
    mockInstancesApi([]);
    render(<InstancesSection type="radarr" />);

    expect(await screen.findByText(/no radarr instances configured yet/i)).toBeInTheDocument();
  });
  ...
  it("has no type selector on the create form", async () => {
    ...
    await screen.findByText(/no radarr instances configured yet/i);
  ...
  it("creates a new instance via the form, fixed to type radarr", async () => {
    const fetchMock = mockInstancesApi([]);
    render(<InstancesSection type="radarr" />);
    ...
    fireEvent.change(screen.getByLabelText("Name"), { target: { value: "Radarr Main" } });
    ...
    expect(await screen.findByText("Radarr Main")).toBeInTheDocument();
    expect(fetchMock).toHaveBeenCalledWith(
      "/api/instances",
      expect.objectContaining({
        method: "POST",
        body: expect.stringContaining('"type":"radarr"'),
      }),
    );
  });
  ...
  it("edits an existing instance", async () => {
    ...
    const row = (await screen.findByText("Radarr Prod")).closest("tr") as HTMLElement;
    fireEvent.click(within(row).getByRole("button", { name: /^edit$/i }));

    const nameInput = screen.getByLabelText("Edit instance name");
    fireEvent.change(nameInput, { target: { value: "Radarr Main" } });
  ...
  it("manages path mappings: adds one and shows it in the list", async () => {
    ...
    await screen.findByText("Radarr Prod");
    ...
    fireEvent.change(screen.getByLabelText(/remote prefix/i), { target: { value: "/movies" } });
    fireEvent.change(screen.getByLabelText(/local prefix/i), { target: { value: "/mnt/movies" } });
