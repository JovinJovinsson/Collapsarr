"""Tests for the ``auto_queue_paused`` migration (COL-174, "Auto-Queuing Pause").

Mirrors ``test_migration_log_level.py``'s "post-baseline delta" idiom: build a
...
#: The revision immediately prior to COL-174's migration (COL-167's
#: ``recently_processed_window_minutes`` column).
_PRIOR_REVISION = "b01929bfdfa4"
...
def _create_pre_migration_global_settings_row(settings: Settings) -> None:
    ...
    Models an already-deployed install about to take the COL-174 update: its
    ``global_settings`` row predates the ``auto_queue_paused`` column
    entirely.
    """
...
def test_migration_backfills_existing_row_to_false(settings: Settings) -> None:
    """An existing install's row gets ``auto_queue_paused = 0`` (not paused)."""
    ...
    try:
        with engine.connect() as connection:
            paused = connection.execute(
                text("SELECT auto_queue_paused FROM global_settings WHERE id = 1")
            ).scalar_one()
...
def test_fresh_install_has_auto_queue_paused_column_defaulting_to_false(
    settings: Settings,
) -> None:
